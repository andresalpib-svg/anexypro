...
CREATE INDEX "charges_condominium_id_status_due_date_idx" ON "charges"("condominium_id", "status", "due_date");

-- Pagos del condominio por fecha; es además la columna por la que
-- filtra su propia política de RLS.
CREATE INDEX "payments_condominium_id_payment_date_idx" ON "payments"("condominium_id", "payment_date" DESC);
...
CREATE INDEX "condominium_supervisors_user_id_idx" ON "condominium_supervisors"("user_id");

-- Asambleas: puntos y adjuntos se leen siempre por asamblea.
CREATE INDEX "assembly_topics_assembly_id_idx" ON "assembly_topics"("assembly_id");
CREATE INDEX "assembly_attachments_assembly_id_idx" ON "assembly_attachments"("assembly_id");

-- Destinatarios de un comunicado.
CREATE INDEX "communication_targets_communication_id_idx" ON "communication_targets"("communication_id");
